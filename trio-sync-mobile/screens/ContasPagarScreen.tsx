...
import { Transacao, TransacaoFormData, CATEGORIAS_DESPESA, STATUS_OPTIONS } from '../types';
...
export default function ContasPagarScreen() {
    ...
    const handleEdit = (item: Transacao) => {
        ...
        setFornecedor(item.fornecedor_cliente || '');
    ...
    const handleRegistrarPagamento = async (item: Transacao) => {
        try {
            const { error } = await supabase
                .from('transacoes')
                .update({
    ...
    const getSummary = () => {
        ...
        const pago = transacoes.filter(t => t.status === 'pago').reduce((sum, t) => sum + (t.valor || 0), 0);
        ...
        <View style={styles.container}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.summaryContainer}>
                ...
                <View style={[styles.summaryCard, { backgroundColor: Colors.success }]}>
                    ...
                    <Text style={styles.summaryValue}>{formatCurrency(summary.pago)}</Text>
                </View>
                            ...
                            {item.status === 'pendente' && (
                                <View style={styles.actionButtons}>
                                    <TouchableOpacity
                                        ...
                                        onPress={() => handleRegistrarPagamento(item)}
                                    >
                                        <Feather name="check" size={16} color="#fff" />
                                        <Text style={styles.smallButtonText}>Registrar Pagamento</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
